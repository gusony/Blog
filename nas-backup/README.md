...
| `./nas_backup.sh backup [--force]` | 執行單向備份（rsync，僅覆寫較新檔案） |

## 備份邏輯

- 使用 `rsync --update`：目的地已有且較新的檔案會略過
- 備份前檢查：計算來源總大小 vs 本地可用空間
- 空間不足時：發送通知（Telegram/Email）並中止
- 增量備份且確認空間足夠時：`./nas_backup.sh backup --force` 略過檢查
- 細碎小檔案較多時：`./nas_backup.sh backup --parallel` 多路徑同時備份可加速
